/**
 * home.view.js — VIEW LAYER
 * Menyusun seluruh section halaman Home.
 * PERUBAHAN (i18n): teks statis memakai t(); field data model { id, en }
 * (leadership.role, whyChooseUs.description) memakai pick().
 */
...
/** @deprecated Stat Bar tidak lagi dipanggil di home.controller.js */
export function renderStatBar(stats) {
  const items = stats
    ...
    <div class="text-center px-4">
      ...
      <div class="text-white/50 text-xs font-poppins mt-1 tracking-wider uppercase">${pick(s.label)}</div>
    </div>`
    )
    .join('');
  ...
  return `
    <div class="stat-bar py-6">
      <div class="max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 divide-x divide-gold/10">${items}</div>
      </div>
    </div>
  `;
}
...
export function renderAboutBrief({ leadership }) {
  const leaderCards = leadership
    ...
    <div class="gold-border rounded p-4">
      ...
      <div class="text-white/50 text-xs font-poppins">${pick(l.role)}</div>
    </div>`
    )
    .join('');
    ...
    <section class="py-24 bg-dark-section section-dark">
      <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-16" data-aos="fade-up">
          ...
          <p class="text-white/50 font-poppins text-sm max-w-xl mx-auto leading-7">${t('home.servicesDesc')}</p>
        </div>
...
/** Delapan Pilar Utama — title/subtitle bilingual permanen, tidak pakai pick() */
    ...
    <section class="py-24 core-values-bg section-dark">
      <div class="max-w-7xl mx-auto px-6">
        <div class="text-center mb-16" data-aos="fade-up">
          ...
          <p class="text-white/50 font-poppins text-sm max-w-xl mx-auto leading-7 mt-4">${t('home.pillarDesc')}</p>
        </div>
...
export function renderWhyChooseUsSection(whyChooseUs) {
  const cards = whyChooseUs
    ...
    <div class="why-card" data-aos="fade-up">
      ...
      <div class="why-card-desc">${pick(w.description)}</div>
    </div>`
    )
    .join('');
  ...
  return `
    <section class="py-20 relative overflow-hidden cta-banner-bg">
      ...
      <div class="max-w-4xl mx-auto px-6 text-center relative z-10" data-aos="fade-up">
        ...
        <p class="text-white/50 font-poppins text-sm leading-8 mb-10 max-w-xl mx-auto">${t('home.ctaDesc')}</p>
...
export function renderGalleryStrip(images) {
  ...
  const imgTags = allImages.map((src) => `<img src="${src}" class="marquee-img" alt="" loading="lazy" />`).join('');
